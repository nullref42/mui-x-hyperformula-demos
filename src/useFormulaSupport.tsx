...
export function useFormulaSupport(
  ...
  // Version counter to force re-render after HF mutations
  const [hfVersion, setHfVersion] = React.useState(0);

  // Track row ID order — HyperFormula moveRows physically moves data
  const rowIdsRef = React.useRef<GridRowId[]>(
    initialData.map((row, i) => getRowId?.(row, i) ?? i),
  );
  ...
  const rows = React.useMemo<HFRow[]>(() => {
    ...
    return Array.from({ length: rowCount }, (_, rowIndex) => {
      const rowId = rowIdsRef.current[rowIndex] ?? rowIndex;
      return {
        id: rowId,
   ...
   * This ensures all formula references are updated automatically by HyperFormula.
   */
  const moveRow = useEventCallback((sourceIndex: number, targetIndex: number) => {
    ...
    // HyperFormula moveRows physically moves row data and updates all formula references
    hf.moveRows(sheetId, [sourceIndex], targetIndex);

    // Update our row ID tracking to match the new physical order
    const ids = rowIdsRef.current;
    const [movedId] = ids.splice(sourceIndex, 1);
    const adjustedTarget =
      targetIndex > sourceIndex ? targetIndex - 1 : targetIndex;
    ids.splice(adjustedTarget, 0, movedId);
